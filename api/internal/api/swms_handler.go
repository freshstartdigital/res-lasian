...
func (api *APIHandler) SWMSSchemaHandler(w http.ResponseWriter, r *http.Request) {

	//array of models.SWMSTableData
	...
	json.NewEncoder(w).Encode(swmsSchema)
}
...
type CreateSwmsRequest struct {
	...
	SwmsData       json.RawMessage `json:"swms_data"`
}

func (api *APIHandler) CreateSWMSHandler(w http.ResponseWriter, r *http.Request) {
	var req CreateSwmsRequest
	...
	// Create the SWMS
	swmsID, err := repository.CreateSWMS(api.DB, req.OrganisationID, req.Name, req.SwmsType, req.AccountEmail, req.SwmsData)
	if err != nil {
		log.Println("Error creating SWMS", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}

	// Respond with the SWMS ID
	w.WriteHeader(http.StatusOK)
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(swmsID)
}

func (api *APIHandler) GetAllSWMSByOrgHandler(w http.ResponseWriter, r *http.Request) {
	// Get the organisation ID from the query parameters
	vars := mux.Vars(r)
	var organisationID string
	var err error
	organisationID, ok := vars["organisationID"]
	if !ok {
	...
	swms, err := repository.GetAllByOrg(api.DB, organisationID)
	if err != nil {
